...
export const useCustomFrameProcessor = (
    ...
    const frameProcessor = useFrameProcessor((frame) => {
        ...
        // Initialize defaults for all analysis results
        let blurAnalysis = { isBlurry: false, blurScore: 100, laplacianVariance: 0 };
        let focusAnalysis = { needsFocus: false };
        let exposureAnalysis = { isUnderexposed: false, isOverexposed: false, exposureLevel: 0.5, dynamicRange: 0, clippedHighlights: 0, crushedShadows: 0, recommendation: '' };
        ...
        let histogramStats = { mean: 128, std: 0, median: 128, mode: 128 };
        ...
        try {
            ...
            if (!cv || !cv.frameBufferToMat || !resize) {
                // If dependencies missing, we still want to report "empty" analysis so UI doesn't stale
                ...
                if (matReady) {
                    try {
                        ...
                        for (let i = 0; i < loopLimit; i++) {
                            ...
                            const vertices = cv.invoke('rows', approx);
